  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 flex justify-between items-center relative">
        ...
        >
          <div
            className="absolute inset-0 bg-gradient-to-br from-black/85 via-stone-950/85 to-primary/55 backdrop-blur-lg"
            onClick={closeMenu}
          ></div>
        </div>
        ...
        <nav
          id="mobile-menu"
          aria-label="行動版導覽"
          className={`lg:hidden fixed top-0 left-0 z-[50] h-full w-[85%] max-w-sm bg-gradient-to-b from-stone-950 via-stone-900 to-primary/70 shadow-2xl border-r border-primary/30 py-24 px-7 flex flex-col gap-8 transition-transform duration-300 ease-out text-white ${
          ...
          <div className="space-y-6">
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.hint && <p className="mt-2 text-sm text-white/80 leading-relaxed">{link.hint}</p>}
              </Link>
            ))}
          </div>
